    ...
    try {
      const res = await fetch(FORMSUBMIT_URL, {
        ...
        body: JSON.stringify({
          ...
          _subject: `${c.subjectPrefix} ${form.name.trim()}`,
          _template: "table",
          _captcha: "false",
        }),
      });
      ...
      if (failed) {
        throw new Error(data.message || c.errorTitle);
      }
    ...
    <section id="contact" className="section-padding relative">
      <div className="container-xl">
        <div className="reveal max-w-2xl">
          ...
          <h2 className="section-title mt-3">{c.heading}</h2>
          <p className="mt-4 text-white/65">{c.intro}</p>
        </div>
          ...
          <div className="md:col-span-5 space-y-4 reveal">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-white/55 text-xs mono">{c.email}</div>
            ...
            <div className="card-surface rounded-xl p-5 flex items-center gap-4">
              ...
              <div>
                <div className="text-white/55 text-xs mono">{c.location}</div>
                <div className="text-white text-sm">{t.personal.location}</div>
              </div>
            </div>
          ...
          {sent ? (
            ...
            >
              ...
              <div className="mt-6 flex items-center justify-between gap-4 flex-wrap">
                <p className="text-xs text-white/45">{c.hint}</p>
                <button
                  type="submit"
                  disabled={loading}
